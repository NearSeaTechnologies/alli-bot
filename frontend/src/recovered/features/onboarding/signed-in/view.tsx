...
export function SignedInOnboardingStep({ step, draft, dailyTools, meetBeat = 0, demoBeat = 0, jobsBeat = 0, error = null, computer = EMPTY_COMPUTER, isAvatarLanded = false, renderCharacter, onAvatarSlotAttached, onDraftChange, onDailyToolsChange, onBack, onForward, onCreate }: {
  ...
  if (step === "meet") return <StepLayout step={step} title="Meet Alli Bot" footer={<Footer onForward={onForward} />}><div aria-hidden="true" className="sand-onboarding__composer"><p>{typedMeetText(meetBeat)}{meetBeat >= ONBOARDING_MEET_DELAY_BEATS && meetBeat < ONBOARDING_MEET_DELAY_BEATS + MEET_TYPED_TEXT.length ? <span style={{ borderLeft: "1px solid currentColor", display: "inline-block", height: "1em" }} /> : null}</p><span><button aria-label="Attach" tabIndex={-1} type="button">+</button><button aria-label="Send" onClick={onForward} tabIndex={-1} type="button">↑</button></span></div></StepLayout>;
  if (step === "computer-demo") return <StepLayout step={step} title="Alli Bot has its own computer and works just like you" footer={<Footer onBack={onBack} onForward={onForward} />}><ComputerDemo beat={demoBeat} /></StepLayout>;
  ...
  return <div className="sand-onboarding__hand-off"><span aria-hidden="true"><img alt="" className="sand-1lliihq sand-2lah0s sand-47corl sand-87ps6o sand-10xuot4" draggable={false} height={APP_ICON_SIZE_PX} src={APP_ICON_URL} width={APP_ICON_SIZE_PX} /><span>Alli Bot</span></span><p aria-live="polite" id={`sand-onboarding-${step}-heading`}>{error == null ? handOffStatus(computer) : "Alli Bot couldn’t finish setting up"}</p>{error == null ? null : <><p>{error}</p><button autoFocus onClick={onCreate} type="button">Try again</button></>}</div>;
}
