...
test("Router settings use the trusted backend and display recorded inference usage", async (t) => {
  ...
  assert.match(mainEdge, /mode === "local-docker"\) await startLocalDockerBox\(settingsPath\); else \{ await stopLocalDockerBox\(\); await connectSandboxBox\(settingsPath\); \}/);
  assert.doesNotMatch(mainEdge, /setBoxRuntime", "sandbox"/);
  const settingsStore = await readFile(path.join(repoRoot, "source", "shared", "node", "settings", "sand-settings-store.ts"), "utf8");
  assert.match(settingsStore, /migrateLegacyRemoteBoxRuntime\(\)/);
