import { copyFile, open, mkdir, readFile, rm, stat, writeFile } from "node:fs/promises";
import { basename, extname, join } from "node:path";

import { coerceAttachmentBytes } from "../../shared/media/attachment-desktop-args.js";
...
export interface AttachmentEdgeDeps {
  ...
  readonly showOpenDialog: (window: unknown | null, options: { properties: readonly string[]; filters?: readonly { name: string; extensions: readonly string[] }[] }) => Promise<{ canceled: boolean; filePaths: readonly string[] }>;
...
export function createAttachmentEdgePort(deps: AttachmentEdgeDeps) {
  ...
  return {
    async pickComposerFiles() {
      try {
        const prompt = await deps.showOpenDialog(deps.getMainWindow(), { properties: ["openFile", "multiSelections"] });
        if (prompt.canceled) return [];
        const picked: { ok: true; path: string; name: string; size: number }[] = [];
        const dir = deps.getStagingDir();
        await mkdir(dir, { recursive: true });
        for (const filePath of prompt.filePaths) {
          const name = basename(filePath);
          if (!isSafeFilename(name)) continue;
          let info: Awaited<ReturnType<typeof stat>>;
          try { info = await stat(filePath); } catch { continue; }
          if (!info.isFile() || info.size === 0) continue;
          if (info.size > deps.byteLimitForName(name)) continue;
          const stagedPath = join(dir, `${(deps.now ?? Date.now)()}-${(deps.randomUUID ?? crypto.randomUUID)()}${extname(name)}`);
          try { await copyFile(filePath, stagedPath); } catch (error) { report("pick", error); continue; }
          picked.push({ ok: true, path: stagedPath, name, size: info.size });
    ...
    async stageBytes(filename: unknown, bytes: unknown) {
      const payload = coerceAttachmentBytes(bytes);
      if (!isSafeFilename(filename) || payload == null) return { ok: false as const, reason: "failed" as const };
      if (payload.byteLength === 0) return { ok: false as const, reason: "empty" as const };
      if (payload.byteLength > deps.byteLimitForName(filename)) return { ok: false as const, reason: "too-large" as const };
      try { const dir = deps.getStagingDir(); await mkdir(dir, { recursive: true }); const path = join(dir, `${(deps.now ?? Date.now)()}-${(deps.randomUUID ?? crypto.randomUUID)()}${extname(filename)}`); await writeFile(path, Buffer.from(payload)); return { ok: true as const, path }; } catch (error) { report("stage", error); return { ok: false as const, reason: "failed" as const }; }
    },
    async commitStaged(rawPaths: unknown, rawFilenames: unknown): Promise<string[] | null> {
      ...
      for (let index = 0; index < paths.length; index += 1) {
        ...
        if (typeof stagedPath !== "string" || stagedPath.length === 0 || !isSafeFilename(filename) || !deps.isWithinStagingDir(stagedPath)) return null;
        let bytes: Buffer;
        try { bytes = await readFile(stagedPath); } catch (error) { report("commit", error); return null; }
        if (bytes.byteLength === 0) return null;
        try {
          committed.push((await deps.legs.uploadAttachment({ filename, bytesBase64: bytes.toString("base64") })).path);
        } catch (error) {
          report("commit", error);
          committed.push(stagedPath);
        }
      }
      return committed;
    },
