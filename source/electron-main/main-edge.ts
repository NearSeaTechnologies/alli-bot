...
import { coerceSelectableInferenceProvider, isSelectableInferenceProvider } from "../shared/inference-router.js";
import { getLocalInferenceCliStatus } from "../shared/node/inference-router-local.js";
import { isSandBoxRuntime, resolveSandBoxRuntime } from "../shared/box-runtime.js";
import { connectSandboxBox, getLocalDockerStatus, getSandboxStatus, startLocalDockerBox, stopLocalDockerBox } from "./box/local-docker-host-connector.js";
...
export function createMainEdgeHandlers(deps: MainEdgeDeps): HandlerMap {
  const handlers: HandlerMap = {
    ...
    getInferenceRouter: async () => { const settings = await deps.readHostSettingsFromBox().catch(() => ({} as UnknownRecord)); const provider = coerceSelectableInferenceProvider(invoke(deps.settingsStore, "getInferenceProvider")); return { provider, usage: settings.inferenceRouterUsage ?? invoke(deps.settingsStore, "getInferenceRouterUsage") ?? null, local: getLocalInferenceCliStatus() }; },
    setInferenceRouter: async (raw) => { const provider = req(raw).provider; invariant(isSelectableInferenceProvider(provider), "Unknown inference provider."); invoke(deps.settingsStore, "setInferenceProvider", provider); const settings = await deps.syncHostSettingsToBox({ inferenceProvider: provider }).catch(() => null); return { provider, usage: settings?.inferenceRouterUsage ?? invoke(deps.settingsStore, "getInferenceRouterUsage") ?? null, local: getLocalInferenceCliStatus() }; },
    getBoxRuntime: async () => { let mode = invoke(deps.settingsStore, "getBoxRuntime"); if (mode !== "local-docker") { if (mode !== "sandbox") invoke(deps.settingsStore, "setBoxRuntime", "sandbox"); mode = "sandbox"; } invariant(isSandBoxRuntime(mode), "Unknown box runtime."); const settingsPath = String(Reflect.get(deps.settingsStore, "settingsPath")); return { mode, status: mode === "sandbox" ? await getSandboxStatus(settingsPath) : await getLocalDockerStatus(settingsPath) }; },
    setBoxRuntime: async (raw) => { const mode = resolveSandBoxRuntime(req(raw).mode); invariant(isSandBoxRuntime(mode), "Unknown box runtime."); const settingsPath = String(Reflect.get(deps.settingsStore, "settingsPath")); const previous = invoke(deps.settingsStore, "getBoxRuntime"); invoke(deps.settingsStore, "setBoxRuntime", mode); try { if (mode === "local-docker") await startLocalDockerBox(settingsPath); else if (mode === "sandbox") { await stopLocalDockerBox(); await connectSandboxBox(settingsPath); } else await stopLocalDockerBox(); } catch (error) { invoke(deps.settingsStore, "setBoxRuntime", previous); throw error; } invoke(deps.boxRecovery, "restartCoordinator"); return { mode, status: mode === "sandbox" ? await getSandboxStatus(settingsPath) : await getLocalDockerStatus(settingsPath) }; },
