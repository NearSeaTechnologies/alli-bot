...
export function createMainEdgeHandlers(deps: MainEdgeDeps): HandlerMap {
  const handlers: HandlerMap = {
    ...
    getBoxRuntime: async () => { const mode = invoke(deps.settingsStore, "getBoxRuntime"); invariant(isSandBoxRuntime(mode), "Unknown box runtime."); const settingsPath = String(Reflect.get(deps.settingsStore, "settingsPath")); return { mode, status: mode === "sandbox" ? await getSandboxStatus(settingsPath) : await getLocalDockerStatus(settingsPath) }; },
    setBoxRuntime: async (raw) => { const mode = resolveSandBoxRuntime(req(raw).mode); invariant(isSandBoxRuntime(mode), "Unknown box runtime."); const settingsPath = String(Reflect.get(deps.settingsStore, "settingsPath")); const previous = invoke(deps.settingsStore, "getBoxRuntime"); invoke(deps.settingsStore, "setBoxRuntime", mode); try { if (mode === "local-docker") await startLocalDockerBox(settingsPath); else { await stopLocalDockerBox(); await connectSandboxBox(settingsPath); } } catch (error) { invoke(deps.settingsStore, "setBoxRuntime", previous); throw error; } invoke(deps.boxRecovery, "restartCoordinator"); return { mode, status: mode === "sandbox" ? await getSandboxStatus(settingsPath) : await getLocalDockerStatus(settingsPath) }; },
