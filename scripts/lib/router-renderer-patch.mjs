...
// "beta" is the Updates section, and it carries the only entry point to Reset
// computer / Update computer / Update baseline. Dropping it took box maintenance
// with it, and any navigation to that section silently landed on General.
const REGISTRY_AFTER = 'const wDn=[{id:"general",label:"General",icon:"settings-gear"},{id:"router",label:"Router (Alli)",icon:"git-branch"},{id:"beta",label:"Updates",icon:"cloud-download"}]';
...
function RBoxRuntime(){const[s,e]=de.useState({status:null,error:null,busy:!0});de.useEffect(()=>{let t=!0;window.desktop.agent.getBoxRuntime().then(n=>{t&&e({status:n?.status??null,error:null,busy:!1})}).catch(n=>{t&&e(r=>({...r,error:String(n?.message??n),busy:!1}))});return()=>{t=!1}},[]);const t=s.busy?"Checking the sandbox computer…":s.status?.detail??"Shell, files and computer use always run on the sandbox computer through localhost:1340.";return a.jsxs("div",{children:[a.jsx(ie,{description:t,label:"Use sandbox computer",variant:"card",children:a.jsx("button",{"aria-checked":!0,"aria-label":"Use sandbox computer",disabled:!0,role:"switch",style:{appearance:"none",background:"var(--color-accent-primary, #4f8cff)",border:0,borderRadius:999,cursor:"default",height:22,opacity:s.busy?0.65:1,padding:2,position:"relative",width:38},type:"button",children:a.jsx("span",{style:{background:"white",borderRadius:"50%",boxShadow:"0 1px 3px rgba(0,0,0,.35)",display:"block",height:18,transform:"translateX(16px)",width:18}})})}),s.error?a.jsx(se,{as:"p",color:"red",size:"sm",children:s.error}):null]})}
...
export function patchOriginalComposerFilePicker(source) {
  ...
  return patchKickstartRetry(patched);
}
